...
HEALTH="${HEALTH:-http://127.0.0.1:30000/health}"
SERVE_LOG="${SERVE_LOG:-$OUT/${ARM}-serve.log}"
...
s=0
while [ $s -lt 2400 ]; do
  [ "$(curl -s --max-time 3 -o /dev/null -w '%{http_code}' "$HEALTH" 2>/dev/null)" = "200" ] && break
  # The scheduler can die while the container stays Up, so watch the log too.
  if grep -q "Scheduler hit an exception" "$SERVE_LOG" 2>/dev/null; then
    echo "SCHEDULER CRASHED at ${s}s:"
    grep -A3 "Scheduler hit an exception" "$SERVE_LOG" | head -4
    grep -E "^[A-Za-z]*(Error|Exception):" "$SERVE_LOG" | tail -3
    exit 1
  fi
  if [ $s -ge 60 ] && ! docker ps --format '{{.Names}}' | grep -qE 'qwen38-sglang-run|qwen3.8-27b-sglang'; then
    echo "CONTAINER DIED at ${s}s. Last lines:"; tail -30 "$SERVE_LOG"; exit 1
  fi
  sleep 10; s=$((s+10))
done
[ $s -ge 2400 ] && { echo "TIMEOUT after ${s}s"; tail -30 "$SERVE_LOG"; exit 1; }
...
grep -oE "accept len: [0-9.]+" "$SERVE_LOG" | tail -40 | awk '{s+=$3;n++} END{if(n)printf "mean accept len over last %d decode batches: %.2f\n",n,s/n; else print "not logged"}'
